    ...
    if (!citiesLocationData[query]) {
        ...
        superagent
            ...
            .then((data) => {
                ...
                let newLocation = new LocationObj(
                    query,
...
function handleWeather(req, res) {
    ...
    let SQL =
        "INSERT INTO weathers (search_query,forecast ,time) VALUES($1, $2, $3) RETURNING *";
            ...
            superagent
                ...
                .then((data) => {
                    ...
                    data.body.data.forEach((ent) => {
                        client.query(SQL, [query, ent.weather.description, ent.valid_date]);
                    });
...
function handleParks(req, res) {
    let query = req.query.search_query;
    let SQL =
        "INSERT INTO parks (search_query,name,address ,fee,description,url) VALUES($1, $2, $3, $4, $5,$6) RETURNING *";
    client
        .query("SELECT * FROM parks WHERE search_query=$1", [query])
        .then((data) => {
            if (data.rowCount == 0) {
                superagent
                    .get(
                        `https://developer.nps.gov/api/v1/parks?q=${query}&api_key=${process.env.PARKS_API_KEY}`
                    )
                    .then((data) => {
...
function handleMovies(req, res) {
    ...
    let SQL2 =
        "INSERT INTO movies (search_query,title,overview ,average_votes,total_votes,image_url,popularity,released_on) VALUES($1, $2, $3, $4, $5,$6,$7,$8) RETURNING *";
    let SQL = "SELECT * FROM movies WHERE search_query=$1";
    client.query(SQL, [query]).then((data) => {
        if (data.rowCount == 0) {
            superagent
                .get(
                    `https://api.themoviedb.org/3/search/movie?api_key=${process.env.MOVIE_API_KEY}&query=${query}`
                )
                .then((data2) => {
                    data2.body.results.filter(thing => thing.poster_path).slice(0, 20).forEach((elem) => {
                        client.query(SQL2, [
                            query,
                            elem.title,
                            elem.overview,
                    ...
                    client
                        .query("SELECT * FROM movies WHERE search_query=$1", [query])
                        .then((data) => {
                            res.send(data.rows);
                        });
                })
...
function handleYelp(req, res) {
    ...
    let SQL =
        "INSERT INTO restaurents (search_query,name,image_url ,price,rating,url) VALUES($1, $2, $3, $4, $5,$6) RETURNING *";
    client
        .query("SELECT * FROM restaurents WHERE search_query=$1", [query])
        .then((data) => {
            if (data.rowCount == 0) {
                superagent
                    .get(
                        `https://api.yelp.com/v3/businesses/search?location=${query}&limit=50`
                    )
                    .set("Authorization", `Bearer ${process.env.YELP_API_KEY}`)
                    .then((data2) => {
                        data2.body.businesses.filter(thing => thing.image_url).forEach((ent) => {
                            client.query(SQL, [
                                query,
                                ent.name,
                                ent.image_url,
                        ...
                        client
                            .query(
                                `SELECT * FROM restaurents WHERE search_query=$1 Limit ${
                  5 * page
                } `,
                                [query]
                            )
